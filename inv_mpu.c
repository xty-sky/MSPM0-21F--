...
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)
{
    ...
    unsigned char more;
    long quat[4]; 
    unsigned char got_quat = 0;
    unsigned char drain_count = 0;

    /*
     * [2026-06-12 主层随机回摆修复]
     * dmp_read_fifo() 每次只取 FIFO 中的一个 packet, more 表示后面还有几包。
     * 主代码层比调试层任务多, 偶发跨过多个 10ms 周期时 FIFO 会积压旧姿态。
     * 若只读第一包, 控制环会拿旧 yaw 做 TURN_ABS 判定和 D 项阻尼, 表现为随机超调/回摆。
     * 这里最多 drain 4 包, 返回本次能拿到的最新四元数；正常无积压时只执行 1 次。
     */
